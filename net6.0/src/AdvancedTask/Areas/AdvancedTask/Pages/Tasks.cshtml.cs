...
using AdvancedTask.Pages.AdvancedTask.Pages.Models;
using AdvancedTask.Business;
using AdvancedTask.Helper;
using AdvancedTask.Models;
using AdvancedTask.Pages.AdvancedTask.Components.Pager;
using EPiServer;
...
namespace AdvancedTask.Pages.AdvancedTask
{
    ...
    public class Tasks : PageModel
    {
        ...
        public Tasks(IApprovalRepository approvalRepository, IContentRepository contentRepository, IContentTypeRepository contentTypeRepository, IUserNotificationRepository userNotificationRepository, IApprovalEngine approvalEngine, LocalizationService localizationService, IAsyncDatabaseExecutor asyncDatabaseExecutor, IChangeTaskHelper changeTaskHelper, IUIHelper helper, IConfiguration configuration)
        {
        ...
        public IPagedList<ContentTask> Items { get; set; } = Enumerable.Empty<ContentTask>().ToPagedList();
        ...
        {
            var items = await GetContentTasks();
            Items = items.ToPagedList(Paging.PageNumber, Paging.PageSize);

            var count = items.Count(x => x.CanUserPublish);
            if (count != 0)
            {
        ...
        //public ActionResult GetTasks(int? pageNumber, int? pageSize, string sorting, string taskValues, string approvalComment, string publishContent, bool? isChange)
        //{
        //    CheckAccess();

        //    var pageNr = pageNumber ?? 1;
        //    var pageSz = pageSize ?? 50;
        ...
        private async Task<List<ContentTask>> GetContentTasks()
        {
            ...
            var list = await _approvalRepository.ListAsync(query, (Paging.PageNumber - 1) * Paging.PageSize, Paging.PageSize);
            ...
            foreach (var task in list.PagedResult)
            {
                ...
                if (task is ContentApproval approval)
                {
                    ...
                    if (content != null)
                    {
                        ...
                        var enableContentApprovalDeadline = _configuration.GetValue<bool>("ATM:EnableContentApprovalDeadline:True");
